...
const { GAME_STATES, MIN_BET, BLIND_INCREASE_INTERVAL } = require('../config/gameConfig');
const { validateAnswer } = require('../utils/validators');

const GameState = GAME_STATES;

class Game {
  constructor(io) {
    this.io = io;
    this.gameId = 'default_quiz_poker_game';
    this.players = {};
    this.playerOrder = [];
  ...
  async startGame(starterSocketId) {
    logGameEvent('START_GAME_CALLED', { starterSocketId, currentHost: this.hostSocketId });
    ...
    // Check for connected players (ignoring hasFolded because it's a new round)
    const connectedPlayers = Object.values(this.players).filter(p => p.role === 'player' && p.isActive);
    logGameEvent('START_GAME_CONNECTED_PLAYERS', { count: connectedPlayers.length, players: connectedPlayers.map(p => p.name) });

    if (connectedPlayers.length < 1) { // Mindestens 1 Spieler für Tests (sonst 2)
      this.io.to(starterSocketId).emit('errorMessage', "Es müssen mindestens 1 Spieler aktiv sein, um das Spiel zu starten!");
      logError(new Error('Not enough players to start game'), { context: 'startGame', connectedPlayerCount: connectedPlayers.length });
      return false; // Indicate failure
    }
  ...
  async submitAnswer(socketId, answer) {
    ...
    if (!validation.isValid) {
      this.io.to(socketId).emit('errorMessage', validation.message);
      return false;
    }

    // Store as number
    const numericAnswer = validation.value;

    player.finalAnswer = numericAnswer;
    player.isAnswerRevealed = false; // Ensure it starts hidden
    logGameEvent('PLAYER_SUBMITTED_ANSWER', { socketId, name: player.name, answer: numericAnswer });
